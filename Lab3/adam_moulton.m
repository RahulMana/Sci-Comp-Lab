function [y,time_domain] = adam_moulton(fy,der_fy,y0,dt,t_end) 
...
%% Newton's method parameters
    d_G = @(Y) 1 - (dt/2)*der_fy(Y);
    for i=2:N
        G = @(Y) Y - y(i-1) - (dt/2)*(fy(Y) + fy(y(i-1)));
        initial_guess = (y(i-1));
        y(i) = newton_method(G,d_G,initial_guess,Accuracy,num_of_iterations);
        
        if isnan(y(i)) 
            fprintf("There is no convergence in AM for time step." + num2str(dt)+"\n");
